...
const Login = () => {
  ...
  const handleLogin = (e) => {
    ...
    setTimeout(() => {
      // Set the token (Note: spelling matches your ProtectedRoute 'adminTocken')
      localStorage.setItem("adminTocken", "mock-jwt-token-12345");
      
      // Set the admin user details for the Topbar
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 p-4">
      ...
      <div className="w-full max-w-md bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 p-8 sm:p-10 relative overflow-hidden">
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full flex items-center justify-center py-3 px-4 bg-brand hover:bg-brand-dark text-white font-bold rounded-xl shadow-md shadow-brand/20 transition-all active:scale-[0.98] disabled:opacity-70 disabled:active:scale-100 mt-8"
          >
            {isLoading ? (
      ...
      <p className="text-xs font-medium text-slate-400 mt-8">
        Secure Admin Portal &copy; {new Date().getFullYear()} GHARMB. All rights reserved.
      </p>
    </div>
  );
};
